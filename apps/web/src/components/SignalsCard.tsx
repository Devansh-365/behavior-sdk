...
function Metric({ label, value, trail, highlight }: { label: string; value: string; trail?: string; highlight?: boolean }): JSX.Element {
  return (
  ...
  return (
    <section className="rounded-xl border border-slate-800/80 bg-slate-900/40 p-5">
      <div className="mb-3 flex items-center justify-between">
        ...
        <span className="font-mono text-[10px] uppercase tracking-wider text-slate-500">250ms refresh</span>
      </div>
      <div className="grid grid-cols-1 gap-x-6 gap-y-4 sm:grid-cols-2">
        ...
        <Section title="Pointer">
          ...
          <Metric label="Curvature variance" value={fmtNum(curvVar, 3)} highlight={(m?.curvature.length ?? 0) >= 5 && curvVar > 0 && curvVar < 0.05} />
          <Metric label="Stillness ratio" value={fmtNum((m?.stillnessRatio ?? 0) * 100, 0)} trail="%" highlight={(m?.pathLength ?? 0) > 5 && (m?.stillnessRatio ?? 0) > 0.7} />
        ...
        <Section title="Paste">
          ...
          <Metric label="Paste ratio" value={fmtNum(p?.pasteRatio ?? 0, 2)} highlight={(p?.pasteRatio ?? 0) > 0.8} />
        </Section>
        <Section title="Corrections / Scroll">
          <Metric label="Backspaces" value={String(cx?.backspaceCount ?? 0)} />
          <Metric label="Correction ratio" value={fmtNum(cx?.correctionRatio ?? 0, 3)} highlight={(p?.charCount ?? 0) > 50 && (cx?.correctionRatio ?? 0) === 0} />
        ...
        <Section title="Input origin">
          ...
          <Metric label="Programmatic" value={String(it?.programmatic ?? 0)} highlight={(it?.programmatic ?? 0) > 5 && (it?.typed ?? 0) + (it?.pasted ?? 0) + (it?.dropped ?? 0) === 0} />
        </Section>
        ...
        <Section title="Click precision">
          ...
          <Metric
            label="Mean offset"
            value={(cl?.centerOffsets.length ?? 0) >= 3 ? fmtNum((cl?.centerOffsets ?? []).reduce((s, [dx, dy]) => s + Math.sqrt(dx * dx + dy * dy), 0) / (cl?.centerOffsets.length ?? 1), 1) : '—'}
            trail={(cl?.centerOffsets.length ?? 0) >= 3 ? 'px' : undefined}
            highlight={(cl?.targeted ?? 0) >= 3 && (cl?.centerOffsets.length ?? 0) >= 3 && (cl?.centerOffsets ?? []).reduce((s, [dx, dy]) => s + Math.sqrt(dx * dx + dy * dy), 0) / (cl?.centerOffsets.length ?? 1) < 3}
          />
        </Section>
        <Section title="Session rhythm">
          ...
          <Metric label="Mean gap" value={fmtNum(rhy?.meanBurstGapMs ?? 0, 0)} trail="ms" highlight={(rhy?.burstCount ?? 0) > 3 && (rhy?.meanBurstGapMs ?? 0) > 800} />
          <Metric label="Gap variance" value={fmtNum(rhy?.gapVariance ?? 0, 0)} highlight={(rhy?.burstCount ?? 0) > 3 && (rhy?.gapVariance ?? 0) < 50000 && (rhy?.meanBurstGapMs ?? 0) > 800} />
        </Section>
        <Section title="File upload">
          <Metric label="Via picker" value={String(up?.pickerCount ?? 0)} />
          <Metric label="Via drag-drop" value={String(up?.dragDropCount ?? 0)} />
          <Metric label="Programmatic" value={String(up?.programmaticCount ?? 0)} highlight={(up?.programmaticCount ?? 0) > 0} />
          <Metric label="Files attached" value={String(up?.filesAttached ?? 0)} />
          {(up?.exifResults ?? []).map((r, i) => (
            <Metric key={i} label={`File ${i + 1} (${r.fileType})`} value={r.aiGenerated ? 'AI-generated' : r.metadataEmpty ? 'no metadata' : r.software ?? 'ok'} highlight={r.aiGenerated || (r.fileType === 'jpeg' && !r.hasExif)} />
          ))}
        </Section>
        <Section title="Field timing">
          <Metric label="Fields visited" value={String(ft?.totalFields ?? 0)} />
          <Metric label="Instant fills" value={String(ft?.instantFills ?? 0)} trail="<100ms" highlight={(ft?.instantFills ?? 0) >= 2} />
          {Object.entries(ft?.fieldDwells ?? {}).slice(0, 4).map(([name, dwells]) => (
            <Metric key={name} label={name} value={String(Math.round(dwells.reduce((a, b) => a + b, 0) / Math.max(dwells.length, 1)))} trail="ms avg" highlight={dwells.some(d => d < 100)} />
          ))}
        </Section>
      </div>
    </section>
