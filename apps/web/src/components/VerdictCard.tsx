...
function VerdictIcon({ kind, className }: { kind: Verdict['kind']; className: string }): JSX.Element {
  switch (kind) {
    case 'human':   return <User className={className} strokeWidth={1.75} />
    case 'headless': return <Eye className={className} strokeWidth={1.75} />
    case 'scripted': return <Bot className={className} strokeWidth={1.75} />
    case 'llm':      return <Brain className={className} strokeWidth={1.75} />
    case 'multiple': return <AlertTriangle className={className} strokeWidth={1.75} />
    default:         return <Loader2 className={`${className} animate-spin`} strokeWidth={1.75} />
  }
}
      ...
      <div className="relative">
        <div className="flex flex-col gap-6 sm:flex-row sm:items-start sm:justify-between">
          <div className="flex items-start gap-4">
            <div className={`flex size-14 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br ring-1 ring-inset ${tone.iconBg}`}>
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                {!isAnalyzing && (
                  <span className={`rounded-full px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wider ring-1 ring-inset ${tone.badge}`}>
          ...
          <div className="mt-5 border-t border-slate-800/60 pt-4">
            ...
            <ul className="space-y-1.5">
              {verdict.reasons.map((reason, i) => (
                <li key={i} className="flex items-start gap-2 font-mono text-xs leading-relaxed text-slate-300">
