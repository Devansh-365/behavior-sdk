...
function Pill({ label, on }: { label: string; on: boolean }): JSX.Element {
  return (
    <span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 font-mono text-[11px] ring-1 ring-inset ${on ? 'bg-rose-500/15 text-rose-300 ring-rose-500/30' : 'bg-slate-800/40 text-slate-400 ring-slate-700/40'}`}>
  ...
  return (
    <section className="rounded-xl border border-slate-800/80 bg-slate-900/40 p-5">
      <div className="mb-3 flex items-center justify-between">
        ...
        <span className="font-mono text-[10px] uppercase tracking-wider text-slate-500">cached</span>
      </div>
      ...
      <div className="mt-4 space-y-1.5 border-t border-slate-800/60 pt-3">
        <div className="flex items-start justify-between gap-3 text-xs">
          ...
          <span className="text-right font-mono text-slate-200">{g?.supported ? truncate(g.renderer || g.vendor || 'hidden', 38) : 'unsupported'}</span>
        </div>
        <div className="flex items-center justify-between text-xs">
          <span className="text-slate-400">Audio hash</span>
          <span className="font-mono text-slate-200">{a?.supported ? a.hash || '—' : 'pending'}</span>
        </div>
        <div className="flex items-center justify-between text-xs">
          <span className="text-slate-400">Canvas hash</span>
          <span className="font-mono text-slate-200">{c?.supported ? c.hash || '—' : 'unsupported'}</span>
        </div>
        <div className="flex items-center justify-between text-xs">
          <span className="text-slate-400">Plugins parent / iframe</span>
          <span className="font-mono text-slate-200">{i?.parentPluginCount ?? 0} / {i?.iframePluginCount ?? 0}</span>
        </div>
        <div className="flex items-center justify-between text-xs">
        ...
        <div className="flex items-center justify-between text-xs">
          ...
          <span className={`font-mono ${inc?.isIncognito === true ? 'text-rose-400' : 'text-slate-200'}`}>
            {inc?.isIncognito === null || inc?.isIncognito === undefined ? 'pending' : inc.isIncognito ? `yes (${inc.method})` : `no (${inc.method})`}
          </span>
        </div>
        <div className="flex items-center justify-between text-xs">
