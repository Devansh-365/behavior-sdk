...
Humans make typos and fix them. Long input with a correction ratio of zero is a reliable signal that the form was filled by automation. A bot that deliberately inserts random backspaces to mimic this behavior will produce a different statistical pattern than organic correction.
...
LLM agents typically compose their response in the model's context then paste it into form fields in one or a few large chunks. A high paste ratio across all fields, combined with no scroll events and fast completion, is a strong LLM indicator.
...
Humans almost always scroll while reading or filling out a longer form. Agents that batch-fill fields programmatically rarely scroll at all. Zero scroll events alongside substantial character input is one of the signals used in LLM detection.
...
Every `InputEvent` fired by a real keyboard or clipboard interaction carries an `inputType` string. When a script dispatches a bare `InputEvent` without an `inputType`, it appears as programmatic here. A high programmatic count alongside zero typed, pasted, or dropped events means the form was filled by code that bypassed the browser's input APIs entirely.
...
interface UploadSignals {
  ...
  filesAttached: number      // total files currently attached to the form
...
Tab switching and window focus loss during form filling are normal human behaviors. Agents typically produce neither.
...
An agent that batch-fills a form by setting field values programmatically will produce multiple instant fill events across distinct fields. Two or more instant fills across at least two fields is a reliable LLM-act pattern.
