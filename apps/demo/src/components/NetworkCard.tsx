        ...
        <div className="flex items-baseline gap-2">
          <span
            className={`font-mono text-2xl tabular-nums ${
              subhuman ? 'text-rose-400' : 'text-slate-100'
            }`}
          >
        ...
        {conn?.supported ? (
          <>
            <div className="flex items-center justify-between text-xs">
              ...
              <span className="font-mono text-slate-200">{conn.effectiveType ?? '—'}</span>
            </div>
            <div className="flex items-center justify-between text-xs">
              <span className="text-slate-400">RTT (browser)</span>
              <span className="font-mono text-slate-200">{fmtMs(conn.rtt)}</span>
            </div>
            <div className="flex items-center justify-between text-xs">
              <span className="text-slate-400">Downlink</span>
              <span className="font-mono text-slate-200">{fmt(conn.downlink, ' Mbps')}</span>
            </div>
            <div className="flex items-center justify-between text-xs">
              <span className="text-slate-400">Save-data mode</span>
              <span className="font-mono text-slate-200">
                {conn.saveData === null ? '—' : conn.saveData ? 'on' : 'off'}
              </span>
            </div>
          </>
        ) : (
        ...
        {timing?.supported ? (
          <>
            <div className="flex items-center justify-between text-xs">
              ...
              <span className="font-mono text-slate-200">{fmtMs(timing.dnsMs)}</span>
            </div>
            <div className="flex items-center justify-between text-xs">
              <span className="text-slate-400">TCP connect</span>
              <span className="font-mono text-slate-200">{fmtMs(timing.tcpMs)}</span>
            </div>
            <div className="flex items-center justify-between text-xs">
              <span className="text-slate-400">TLS handshake</span>
              <span className="font-mono text-slate-200">{fmtMs(timing.tlsMs)}</span>
            </div>
            <div className="flex items-center justify-between text-xs">
              <span className="text-slate-400">Time to first byte</span>
              <span className="font-mono text-slate-200">{fmtMs(timing.ttfbMs)}</span>
            </div>
          </>
        ) : (
          <div className="text-xs text-slate-500 italic">
